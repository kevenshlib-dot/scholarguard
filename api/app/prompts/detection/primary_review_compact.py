...
输出格式（严格JSON，无其他内容）：
{{"conf":<0-1>,"lvl":"<low/medium/high/critical>","dim":{{"vd":<1-10>,"sv":<1-10>,"an":<1-10>}},"src":{{"human":<0-1>,"ai":<0-1>,"edited":<0-1>,"humanizer":<0-1>}},"flags":{{"lcd":<0-10>,"slu":<0-10>,"ts":<0-10>}},"segs":[{{"s":<起始字符位置>,"e":<结束字符位置>,"t":"<从原文中直接复制的可疑句子或段落>","i":"<该片段的AI特征说明>"}}],"reason":"<50字以内>","unc":"<30字以内>","rpt":{{"summary":"<一句话>","for":["证据1"],"against":["反向证据1"],"disclaimer":"<结论边界>","actions":["建议1"],"review":<true/false>,"review_why":"<原因或null>"}}}}

判断标准：low=自然人类写作,medium=部分AI特征,high=明显AI特征,critical=确定AI生成
rpt.summary=风险概述,rpt.for=支撑证据,rpt.against=反向证据,rpt.actions=建议操作
segs要求：必须从原文中逐字引用2-5个最可疑的句子/段落，t字段是原文原句（不要改写或总结），i字段说明AI特征原因。如果文本风险低可返回空数组。"""
